    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f172a, #064e3b)", fontFamily: "Arial, sans-serif" }}>
        ...
        <div style={featureGrid}>
          {[feature1, feature2, feature3].map((img, i) => (
            <div key={i} style={darkCard}>
              <img src={img} alt={`Feature ${i + 1}`} style={cardImage} />
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={yellowSection}>
        <div style={yellowContainer}>
          ...
          <div style={howGrid}>
            <HowCardComponent image={step1} title="Post an Errand" />
            <HowCardComponent image={step2} title="Helper Claims an Errand" />
            <HowCardComponent image={step3} title="Task Gets Done" />
          </div>
        </div>
      </section>
    </div>
...
const App = () => {
  return (
    <Routes>
      ...
      <Route path="/errand" element={<PostErrand />} />
